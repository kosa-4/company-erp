...
export default function PurchaseRequestPage() {
    ...
    setSelectedPrItems((prev) =>
      prev.map((item) => {
        const updatedItem = updatedItems.find(
          (updated) => updated.lineNo === item.lineNo,
        );
    ...
    },
    {
      ...
      render: (value, row) => (
        <input
          ...
          onChange={(e) =>
            handleItemChange(row.lineNo, "requestDeliveryDate", e.target.value)
          }
    ...
    },
    {
      ...
      render: (value, row) => (
        <input
          ...
          value={(value as string) || ""}
          onChange={(e) =>
            handleItemChange(row.lineNo, "remark", e.target.value)
          }
          className="w-full border border-gray-300 rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
      ),
  ...
  // 구매유형을 간단한 한글로 변환 (구매요청명 자동완성용)
  const convertPurchaseTypeToSimpleKorean = (purchaseType: string): string => {
    const typeMap: Record<string, string> = {
      GENERAL: "일반",
      CONTRACT: "단가계약",
      URGENT: "긴급",
  ...
  // 구매요청명 자동완성 핸들러
  const handleAutoCompletePrName = () => {
    if (!formData.department) {
      toast.warning("부서 정보가 없습니다.");
      return;
    }

    if (prItems.length === 0) {
      toast.warning("품목을 먼저 추가해주세요.");
      return;
    }
    ...
    let itemPart = "";
    if (prItems.length === 1) {
      // 품목이 1개인 경우: [부서명] + 품목명 + '구매요청 건'
      itemPart = prItems[0].itemName;
    } else {
      // 품목이 여러개인 경우: [부서명] + 1번째품목명 외 N품목 + '구매요청 건'
      const additionalCount = prItems.length - 1;
      itemPart = `${prItems[0].itemName} 외 ${additionalCount}품목`;
    }

    const autoCompletedName = `[${department}] ${itemPart} 구매요청 건`;

    setFormData((prev) => ({ ...prev, prName: autoCompletedName }));
    toast.success("구매요청명이 자동완성되었습니다.");
  };
  ...
  const handleSave = async () => {
    ...
    const invalidItems = prItems.filter(
      (item) =>
        !item.quantity ||
        item.quantity <= 0 ||
        !item.unitPrice ||
    ...
    try {
      ...
      if (uploadedFiles.length > 0) {
        if (!prNum) {
          toast.warning(
            "구매요청 번호를 받지 못해 파일을 업로드할 수 없습니다.",
          );
        } else {
  ...
  const handleRemoveItem = () => {
    ...
    setPrItems(
      prItems.filter((item) => !selectedLineNos.includes(item.lineNo)),
    );
    setSelectedPrItems([]);
  ...
  const handleItemCheckboxChange = (itemCode: string, checked: boolean) => {
    if (checked) {
      ...
      setSelectedItemCodes(
        selectedItemCodes.filter((code) => code !== itemCode),
      );
    }
  };
    ...
    <div>
      ...
      <Card title="기본 정보" className="mb-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Input label="PR번호" value={formData.prNo} readOnly />
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                onChange={(e) =>
                  setFormData((prev) => ({ ...prev, prName: e.target.value }))
                }
                placeholder="구매요청명 입력"
                className="flex-1 h-9 px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-400/30 focus:border-gray-400 transition-colors hover:border-gray-300"
                required
              />
              <Button
                variant="secondary"
                onClick={handleAutoCompletePrName}
          ...
          <Input label="요청자" value={formData.requester} readOnly />
          <Input label="부서" value={formData.department} readOnly />
          <DatePicker label="요청날짜" value={formData.requestDate} disabled />
          <Select
            label="구매유형"
            value={formData.purchaseType}
            onChange={(e) =>
              setFormData((prev) => ({ ...prev, purchaseType: e.target.value }))
            }
            required
            options={[
        ...
        <div className="p-4 bg-gray-50 border-t flex justify-end">
          <div className="text-right">
            ...
            <span className="text-xl font-bold text-blue-600">
              ₩
              {formatNumber(
                prItems.reduce((sum, item) => sum + item.amount, 0),
              )}
            </span>
          </div>
        </div>
                ...
                <tr>
                  <th className="w-10 p-3 text-center">
                    <input
                      ...
                      checked={
                        itemList.length > 0 &&
                        selectedItemCodes.length === itemList.length
                      }
                      onChange={(e) => {
                        if (e.target.checked) {
                          setSelectedItemCodes(
                            itemList.map((item) => item.itemCd),
                          );
                        } else {
                          setSelectedItemCodes([]);
                        }
