...
export default function RfqSelectionResultPage() {
  ...
  // ItemPage 방식: searchParams 변경 → 자동 조회
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      setDetailData({ header: null, items: [] });
    } catch (error) {
  ...
  /** reset 하면 자동 조회(= searchParams 변경) */
  const handleReset = () => {
    setSearchParams({
      rfqNo: '',
      rfqName: '',
          ...
          {detailData.header && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-x-8 gap-y-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                  ...
                  ₩{formatNumber(Number(detailData.header.totalAmt))}
                </span>
                  </div>
                  <div className="flex justify-between border-b border-gray-200 pb-1">
                    <span className="text-gray-500 text-sm">선정일</span>
                    <span className="font-semibold text-gray-900">
                  {detailData.header.selectDate?.substring(0, 10)}
                </span>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="border rounded-lg overflow-hidden">
                    <DataGrid columns={detailColumns} data={detailData.items} keyField="itemCd" />
                  </div>
                </div>
